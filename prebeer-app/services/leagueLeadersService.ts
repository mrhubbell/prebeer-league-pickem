...
export async function getLeagueLeaders(): Promise<LeagueLeaders> {
  ...
  for (const pick of matchPicks ?? []) {
    ...
    const existing = predictionStats.get(pick.member_id) ?? {
      selected: 0,
      correct: 0,
    };
    ...
    predictionStats.set(pick.member_id, existing);
  }

  let matchPredictionLeader: LeagueLeaderStat | null = null;
   ...
   * ---------------------------------------------------------
   *
   * Get all current-season goalscorer picks and actual goals
   * in bulk.
   *
   * The pick is matched to the goal using:
   *   member_id + matchweek_id + player_id
   *
   * This preserves the same Gameweek-specific behavior used
   * by the scoring engine without making separate database
   * calls for every Gameweek.
   */
  ...
  /*
   * Count goals by:
   *   matchweek_id + player_id
   *
   * Only current-season, non-own-goal records are included.
   */
  const goalsByMatchweekPlayer = new Map<string, number>();
  ...
  for (const goal of goals ?? []) {
    ...
    const fixture = goal.fixture as any;
    const matchweekId = fixture?.matchweek_id;
    const seasonId = fixture?.matchweek?.season_id;

    if (
      matchweekId === undefined ||
      seasonId !== currentSeasonId
    ) {
      continue;
    }

    const key = `${matchweekId}-${goal.player_id}`;

    goalsByMatchweekPlayer.set(
      key,
      (goalsByMatchweekPlayer.get(key) ?? 0) + 1
    );
  }
  ...
  for (const pick of goalPicks ?? []) {
    const key = `${pick.matchweek_id}-${pick.player_id}`;

    const goalsScored =
      goalsByMatchweekPlayer.get(key) ?? 0;
   ...
   * ---------------------------------------------------------
   *
   * Same approach as goalscorers:
   *
   *   member_id + matchweek_id + player_id
   *
   * is used to match a member's pick to the actual assists
   * recorded for that player in that specific Gameweek.
   */
   ...
   *   matchweek_id + player_id
   *
   * Only current-season records are included.
   */
  const assistsByMatchweekPlayer = new Map<string, number>();

  for (const assist of assists ?? []) {
    const fixture = assist.fixture as any;
    const matchweekId = fixture?.matchweek_id;
    const seasonId = fixture?.matchweek?.season_id;
    ...
    const key = `${matchweekId}-${assist.player_id}`;

    assistsByMatchweekPlayer.set(
      key,
      (assistsByMatchweekPlayer.get(key) ?? 0) + 1
    );
  }
  ...
  for (const pick of assistPicks ?? []) {
    ...
    const assistCount =
      assistsByMatchweekPlayer.get(key) ?? 0;
